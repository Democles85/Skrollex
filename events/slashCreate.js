...
module.exports = {
	...
	async execute(interaction) {
		...
		if (hasChan) {
			const cmdChannelId = await interaction.client.db.get(
				`${guildId}_cmd_channel`
			);
			if (cmdChannelId !== channelId) {
				return interaction.reply({
					content: `My commands are limited to <#${cmdChannelId}> in your guild!`,
					ephemeral: true,
				});
			}
		}
		...
		if (blRoleId) {
			...
			if (interaction.member.roles.cache.some((role) => role.id === blRole)) {
				return interaction.reply({
					content: "You have been blacklisted from using me!",
					ephemeral: true,
				});
			}
		}
